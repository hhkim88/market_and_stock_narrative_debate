...
def cache_set(target_id, market_id, target_label, results, winner, bull_prob=50, neutral_prob=30, bear_prob=20, status="done"):
    try:
        get_supabase().table("analyses").upsert({"target_id":target_id,"market_id":market_id,"target_label":target_label,"results":results,"winner":winner,"bull_prob":bull_prob,"neutral_prob":neutral_prob,"bear_prob":bear_prob,"status":status,"analyzed_at":datetime.now(timezone.utc).isoformat()},on_conflict="target_id").execute()
...
def load_leaderboard():
    try:
        resp=get_supabase().table("analyses").select("target_id,market_id,target_label,winner,bull_prob,neutral_prob,bear_prob,status,analyzed_at").execute()
        rows=[]
        for r in resp.data:
...
"classifier": f"""{lang}
{c_warn}
당신은 시장 내러티브 분류 전문가입니다.
주어진 【주가 컨텍스트】와 【원자료】를 함께 읽고, 모든 내러티브·사실·이벤트를 두 버킷으로 분류하십시오.

## ⚠️ 분류 판단 기준 (주가 컨텍스트 필수 활용)
아래 신호를 참고하여 분류 정확도를 높이시오:

**Priced-In 판단 신호:**
- 해당 뉴스/이벤트가 수개월 이상 반복 보도됨 + 그 기간 주가가 이미 상승
- 실적 발표 후 주가가 거의 반응 안 했음 → 이미 컨센서스에 반영됐던 것
- 52주 고점 근처에 있음 → 강세 재료는 이미 상당 부분 반영
- 애널리스트 목표가 상향이 이미 수개월 이어져 왔음

**Not Yet Priced-In 판단 신호:**
- 최근 처음 공개된 정보 (어닝콜 신규 언급, 최신 공시)
- 주가가 아직 충분히 반응하지 않은 재료
- 실현 여부가 불확실한 미래 이벤트 (계약 체결 가능성, 규제 변화)
- 시장이 아직 과소평가하는 리스크 (주가가 안 내려갔는데 리스크 존재)
- 52주 저점 근처 + 아직 반영 안 된 회복 촉매

## 🔒 버킷 A: 이미 주가에 반영된 내러티브 (Priced-In)
각 항목: 내용 + "반영 근거: [주가 흐름·보도 반복·컨센서스 기반 판단]"

## 🚀 버킷 B: 아직 주가에 반영되지 않은 미래 내러티브 (Not Yet Priced-In)
각 항목: 내용 + "미반영 근거: [왜 아직 시장이 모르거나 과소평가하는가]" + "실현 시 방향: [상승/하락/중립]"

## 📊 분류 결과 요약
- Priced-In: N개 / Not-Yet-Priced-In: N개
- 전체 무게 중심: [강세 편향 / 중립 / 약세 편향]
- 분류 신뢰도: [높음/보통/낮음] + 신뢰도가 낮은 경우 이유 명시

⚠️ 주의: 정보 부족으로 판단이 어려운 항목은 "분류 불확실"로 표시하고 이유를 밝히시오.
출처를 반드시 명시하시오.{warn}""",

"bull": f"""{lang}
당신은 {target} 강세 케이스 전문 애널리스트입니다.
**미래 미반영 내러티브에 분석의 70% 이상을 집중**하십시오.

## 📈 {target} 강세 내러티브 수집 (향후 3개월)

### 🔒 [참고] 이미 주가에 반영된 강세 요인 (Priced-In)
[분류 결과 중 강세 관련만. 2-3줄로 압축.]
⚠️ 이 항목들은 이미 반영됐으므로 추가 수익률 기여도 낮음.

### 🚀 핵심: 아직 주가에 반영되지 않은 강세 촉매 (Not Yet Priced-In)
각 촉매별로: 촉매 내용 / 왜 아직 미반영인가 / 실현 시 주가 임팩트 방향 / 실현 확률 [높음/중간/낮음]

### 인과 구조 [미반영 촉매들이 강세로 연결되는 이유]
### 정량 근거 점검 [미반영 촉매 지지 수치. 부족하면 "정량 근거 부족" 명시]
### SNS·커뮤니티 정서 [미반영 재료에 대한 시장 인식 수준]
### 어닝콜 핵심 포인트 [미반영 강세 시그널 위주]
### 강세 내러티브 강도 평가 [1-10점] — 미반영 촉매의 질과 양 기준
### 내러티브-정량 일치도 평가 [1-10점]
### 강세 내러티브 3줄 요약 (미반영 요인 중심)
출처(기관명, 날짜, URL)를 반드시 명시하시오.{warn}""",

"neutral": f"""{lang}
당신은 {target} 중립 케이스 전문 애널리스트입니다.
**미래 미반영 내러티브에 분석의 70% 이상을 집중**하십시오.

## ➡️ {target} 중립 내러티브 수집 (향후 3개월)

### 🔒 [참고] 이미 주가에 반영된 중립 요인 (Priced-In)
[이미 반영된 균형 요인. 2-3줄로 압축.]

### 🚀 핵심: 아직 주가에 반영되지 않은 불확실 요인 (Not Yet Priced-In)
각 요인별로: 요인 내용 / 왜 시장이 방향을 못 정했는가 / 해소 트리거와 시점

### 강세·약세 양측이 공통으로 과소평가하는 리스크/기회
### 인과 구조 [박스권이 지속될 수 있는 이유]
### 정량 근거 점검 / SNS·커뮤니티 정서 / 어닝콜 핵심 포인트
### 중립 내러티브 강도 평가 [1-10점] / 내러티브-정량 일치도 [1-10점]
### 중립 내러티브 3줄 요약 (미반영 불확실성 중심)
출처를 반드시 명시하시오.{warn}""",

"bear": f"""{lang}
당신은 {target} 약세 케이스 전문 애널리스트입니다.
**미래 미반영 내러티브에 분석의 70% 이상을 집중**하십시오.

## 📉 {target} 약세 내러티브 수집 (향후 3개월)

### 🔒 [참고] 이미 주가에 반영된 약세 요인 (Priced-In)
[이미 반영된 리스크. 2-3줄로 압축.]
⚠️ 이 항목들은 이미 반영됐으므로 추가 하락 압력 작용 어려움.

### 🚀 핵심: 아직 주가에 반영되지 않은 약세 리스크 (Not Yet Priced-In)
각 리스크별로: 리스크 내용 / 왜 시장이 과소평가하는가 / 현실화 시 임팩트 방향 / 현실화 확률 [높음/중간/낮음]

### 인과 구조 [미반영 리스크들이 약세로 연결되는 이유]
### 정량 근거 점검 [미반영 리스크 지지 수치] / SNS·커뮤니티 정서 / 어닝콜 핵심 포인트
### 약세 내러티브 강도 평가 [1-10점] — 미반영 리스크의 질과 양 기준
### 내러티브-정량 일치도 평가 [1-10점]
### 약세 내러티브 3줄 요약 (미반영 요인 중심)
출처를 반드시 명시하시오.{warn}""",

"bull_critic": f"""{lang}
{c_warn}
## 🔥 강세 내러티브 비판
⚠️ 강세 측이 "미반영"이라 주장한 항목이 실제로 이미 반영됐는지를 집중 비판하시오.

### 미반영이라 주장하지만 실제론 이미 반영됐을 가능성 있는 항목
### 미반영 촉매의 실현 가능성 과장 여부
### 서사의 약한 고리 [인과 연결의 약함]
### 정량 근거의 취약점
### 강세가 무시한 반대 증거 (특히 미반영 리스크)
...
{c_warn}
## 🔥 중립 내러티브 비판
⚠️ 중립이 방향성을 회피한 것인지, 진짜 균형인지를 비판하시오.

### 중립이 단순히 방향성 판단을 회피한 것은 아닌가
### 미반영 요인 중 실제로는 방향성이 명확한 것들
### 서사의 약한 고리 / 정량 근거의 취약점
### 중립이 무시한 비대칭 리스크
...
{c_warn}
## 🔥 약세 내러티브 비판
⚠️ 약세 측이 "미반영"이라 주장한 리스크가 실제로 이미 반영됐는지를 집중 비판하시오.

### 미반영이라 주장하지만 실제론 이미 반영됐을 가능성 있는 항목
### 미반영 리스크의 현실화 확률 과장 여부
### 서사의 약한 고리 / 정량 근거의 취약점
### 약세가 무시한 반대 증거 (특히 미반영 촉매)
...
{c_warn}
## 투자 판정 철학
- **미반영 내러티브(Not Yet Priced-In)가 판정의 핵심**이다.
- Priced-In 내러티브는 현재 주가에 이미 녹아 있으므로 추가 수익률(alpha)에 기여하지 않는다.
- 향후 3개월 주가는 "시장이 아직 모르는 것이 어떻게 실현되는가"에 의해 결정된다.
⚠️ 절대 금지: Priced-In 항목을 판정 주근거로 삼지 말 것 / 목표가 중심 판정 금지 / {cutoff_str_ko} 이전 자료 금지

## 핵심 요약
[4문장: 1.가장 강한 미반영 내러티브. 2.왜 아직 미반영인가. 3.취약한 경쟁 미반영 주장. 4.판단을 뒤집을 핵심 변수.]

## ⚡ 최종 판정

### 가장 그럴듯한 내러티브: [강세 / 중립 / 약세]

### Priced-In vs Not-Yet-Priced-In 분류 요약
**이미 주가 반영 (Priced-In):** [3줄 이내. 현재 주가 수준 설명, 추가 움직임 근거 안 됨.]
**아직 미반영 (Not Yet Priced-In) — 판정의 핵심:** [각 항목 옆 [강세 촉매 / 약세 리스크 / 불확실] 표시]

### 미반영 내러티브 강도 평가
- 강세 측 미반영 촉매 강도: [1-10] / 중립 측 미반영 불확실 강도: [1-10] / 약세 측 미반영 리스크 강도: [1-10]

### 정량 뒷받침 강도
- 강세: [1-10] / 중립: [1-10] / 약세: [1-10]

### 선택 이유
[해당 방향 미반영 내러티브가 설득력 있는 이유. 미반영 이유 + 실현 가능성 + 임팩트 크기를 함께 서술.]
...
### 경쟁 내러티브 탈락 이유 [경쟁 진영의 미반영 주장이 왜 설득력 떨어지는지]
...
### 미반영 내러티브 실현 트리거 (상위 3개)
[각 트리거: 이벤트 / 예상 시점 / 실현 시 주가 방향]""",
    }
...
def display_results(results, winner, cached_at=None):
    ...
    <div style='color:{w_color};font-size:24px;font-weight:900'>{w_label}</div></div>""",unsafe_allow_html=True)
    bp,np_,rp=extract_probs(results.get("judge",""))
    if bp is not None:
        st.markdown("#### 확률 분포")
...
def main():
    ...
    with col_title:
        ...
        ⚡ [시장/종목] 내러티브 앤 넘버스 수집 및 분석 v1.00</h1>
