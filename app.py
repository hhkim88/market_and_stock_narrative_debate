...
def combined_search(target, direction, market_index, sector="", ticker_raw="", market_id="sp500"):
    ...
    naver_section = ""
    if market_id == "kospi200":
        nv = search_naver(target, direction, ticker_raw)
        if nv:
            naver_section = "\n\n" + fmt_items(nv, "⑤ 네이버 금융·뉴스 (Naver API)")
    ...
    src_label = "Tavily+Exa+SNS+어닝콜+네이버" if market_id=="kospi200" else "Tavily+Exa+SNS+어닝콜"
...
def search_naver(target: str, direction: str, ticker: str = "") -> list[dict]:
    """
    네이버 뉴스 검색 API로 한국 금융 정보 수집.
    Tavily·Exa가 놓치는 한국어 뉴스·증권사 리포트·공시 보도를 정확히 가져옴.

    3가지 쿼리로 수집:
    1) 종목명 + 방향별 키워드 (매수/매도/중립 전망)
    2) 종목명 + 실적·재무 키워드
    3) 종목명 + 종토방·커뮤니티 여론
    """
    ...
    if not client_id or not client_secret:
        return []  # 키 없으면 조용히 skip

    dir_keywords = {
        "bull":    ["매수" ,"목표주가 상향", "강세", "상승", "긍정", "추천"],
        "neutral": ["중립", "보합", "관망", "불확실", "횡보"],
        "bear":    ["매도", "목표주가 하향", "약세", "하락", "부정", "리스크"],
    }
    kw = dir_keywords.get(direction, [])
    dir_query = " OR ".join(kw[:3])

    queries = [
        f"{target} {dir_query}",            # 방향별 분석
        f"{target} 실적 영업이익 매출 전망",  # 재무·실적
        f"{target} 주가 전망 증권사",         # 증권사 리포트
    ]
    if ticker:
        queries.append(f"{ticker} {target} 공시 IR")  # 공시·IR
    ...
    for query in queries:
        try:
            ...
            for item in resp.json().get("items", []):
                ...
                title = html.unescape(_re.sub(r"<[^>]+>","", item.get("title","")))
                desc  = html.unescape(_re.sub(r"<[^>]+>","", item.get("description","")))

                # 날짜 파싱 (Naver: "Mon, 07 Apr 2025 10:00:00 +0900")
